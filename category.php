...
<?php 
    ...
    if(isset($_GET['sort'])) {
        ...
        switch ($sort) {
            case 'asc':
                $query = "SELECT * FROM tbl_product WHERE product_name LIKE '%$search%' ORDER BY CAST(product_price_new AS UNSIGNED) ASC";
                break;
            case 'desc':
                $query = "SELECT * FROM tbl_product WHERE product_name LIKE '%$search%' ORDER BY CAST(product_price_new AS UNSIGNED) DESC";
                break;
            default:
                            ...
                            while ($row = $products->fetch_assoc()) {
                              // tao dong` moi
                               echo "</div><div class='product-row'>"; 
                                ...
                                echo "<p>Giá: " . number_format($row["product_price_new"], 0, ',', '.') . "đ</p>"; 
                                echo "</div>";  
